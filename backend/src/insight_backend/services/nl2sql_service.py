    ...
    def explore(
        ...
        *,
        ...
        system = (
            ...
            "Before reusing a literal name from the question (product, team, channel, category, etc.) in a WHERE clause,\n"
            "first propose a DISTINCT query on the candidate column to verify actual values, then reuse only values that\n"
            "exist in the data instead of copying obvious typos from the question.\n"
            "For date parts, use EXTRACT(YEAR|MONTH FROM CAST(CASE WHEN col IS NULL OR col IN ('None','') THEN NULL ELSE col END AS DATE)).\n"
    ...
    def generate_with_evidence(
        ...
        *,
        ...
        system = (
            ...
            "Use the exploration evidence to choose literal filter values (product, team, channel, category, etc.):\n"
            "if a name mentioned in the question does not appear in the evidence, do not use it in WHERE filters and\n"
            "prefer a slightly broader but consistent query based on values that really exist in the data.\n"
            "For date parts, use EXTRACT(YEAR|MONTH FROM CAST(CASE WHEN col IS NULL OR col IN ('None','') THEN NULL ELSE col END AS DATE)).\n"
            "Never use NULLIF with more than 2 arguments; prefer the CASE…END form above.\n"
