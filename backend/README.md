...
- 2025-10-30: NL→SQL – extraction JSON centralisée et garde‑fous d'entrée. Ajout de `_extract_json_blob()` dans `nl2sql_service.py` (remplace la logique de parsing des blocs ```json … ```), validation des paramètres (`question`, `schema`, bornes `max_steps`) et mise sous cap de la taille du prompt (`tables_blob`). Tests: `uv run pytest` → 18 tests OK.
 ...
 - 2025-11-14: NL→SQL multi‑agent — l’Explorateur et l’Analyste ne recopient plus aveuglément les noms de produits/équipes/canaux issus de la question dans les clauses `WHERE`. Les prompts imposent désormais: (1) de vérifier d’abord les valeurs réelles via des `SELECT DISTINCT` d’exploration, (2) de construire la requête finale à partir de valeurs effectivement présentes dans les données, même si la question contient des fautes de frappe. En cas de résultat final à 0 ligne, le backend tente d’identifier les filtres suspects en comparant les littéraux du `WHERE` avec les catégories observées et renvoie un indice explicite dans la réponse (valeurs proches suggérées) au lieu d’un simple « 0 ligne » opaque. Tests: `uv run pytest` → à mettre à jour après exécution.
