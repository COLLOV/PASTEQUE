...
- Garde‑fous configurables dans `.env.example`: `LOOP_MAX_TICKETS` (échantillon par période, défaut 60), `LOOP_TICKET_TEXT_MAX_CHARS` (tronque chaque ticket, 360), `LOOP_MAX_WEEKS` (8), `LOOP_MAX_MONTHS` (6), `LOOP_TEMPERATURE` (0.3), `LOOP_MAX_TOKENS` (800). Quota via `AGENT_MAX_REQUESTS` clé `looper`.
