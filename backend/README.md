...
- La configuration du serveur reste déclarative (`plan/Z/mcp.config.example.json` ou votre `plan/Z/mcp.config.json` local non versionné via `MCP_CONFIG_PATH`), et supporte les variables `VIS_REQUEST_SERVER`, `SERVICE_ID`, etc.
