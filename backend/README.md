...
- `ANIMATION=true`: ajoute un agent léger « animator » qui observe les évènements et émet des messages courts `anim` destinés à expliquer la progression (ex: « Tables actives: N », « Comptage par catégorie », « Résultats: 20 lignes »). Comme `false`, ce mode masque aussi `plan`/`sql` côté SSE et évite leur persistance; seuls les `meta` utiles et l'évidence sont conservés.
