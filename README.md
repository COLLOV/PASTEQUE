...
- Les requêtes Cypher exécutées sur Neo4j sont normalisées côté backend : les comparaisons sur les champs de type date (`creation_date`, `date_feedback`, `last_seen`, etc.) transforment automatiquement les littéraux ISO en `date('YYYY-MM-DD')` pour éviter tout décalage entre les métadonnées et les types réels, et la réponse textuelle est recalculée par le modèle sur la requête normalisée lorsqu’un ajustement est appliqué pour que le chat reste cohérent avec les données affichées.
- Les clauses `LIMIT` proposées par le modèle sont supprimées avant exécution et, lorsqu’une requête retourne uniquement un agrégat (ex. `count`), une requête dérivée `RETURN *` est exécutée pour alimenter le panneau latéral avec l’intégralité des tickets concernés (colonnes aplaties sur les propriétés Neo4j).
