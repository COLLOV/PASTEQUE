...
- Les serveurs MCP exécutés via stdio sont enveloppés par `backend/src/insight_backend/services/mcp_stdio_filter.py` afin de rediriger les journaux non-JSON (ex : « Saving chart to: … ») vers `stderr`, ce qui évite les erreurs JSON-RPC côté client.
