...
- Les logs du serveur MCP sont reroutés côté backend vers STDERR pour garantir que STDOUT reste strictement réservé aux messages JSON-RPC attendus par le client.
- Les paramètres réseau sensibles (ex. `NODE_TLS_REJECT_UNAUTHORIZED=0` pour un proxy interceptant) se configurent via la clé `env` du MCP config; n’injectez plus de `-e` dans `args` pour éviter que `docker run` ne casse le montage `-v`.
