...
- Nouveau mode « Tickets » (bouton étincelle à côté du graphique) : injecte les tickets (date/id/description) dans le prompt via une plage de dates sélectionnable. Les garde‑fous du looper s’appliquent (`LOOP_MAX_TICKETS`, `LOOP_MAX_TICKETS_PER_CALL`, `LOOP_MAX_INPUT_CHARS`): si le volume dépasse le cap, les tickets sont chunkés puis fusionnés en « résumés de résumés ». Le contexte est visible dans le panneau de tickets et reste borné à la plage choisie. Endpoint utilitaire: `GET /api/v1/tickets/context/metadata` pour récupérer les bornes min/max.
