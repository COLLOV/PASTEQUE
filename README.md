...
- Une section « Options de debug » permet d’activer l’affichage du contenu des tickets sélectionnés par le RAG directement dans les détails du chat. Une fois l’option cochée, la bulle de réponse expose table, similarité et colonnes du ticket retenu sous le bouton **Détails**, afin de faciliter l’investigation des conversations.
...
#### Mode Multi‑agent (Explorateur + Analyste + RAG + Rédaction)
...
- Déroulé:
  ...
  - RAG (context): calcule les embeddings de la question utilisateur et des tables autorisées, puis renvoie les `N` lignes les plus similaires (cosinus) au rédacteur. Ces lignes sont injectées dans l'évidence avant la synthèse pour fournir un rappel métier immédiat.
