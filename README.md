...
- La configuration du serveur (`VIS_REQUEST_SERVER`, `SERVICE_ID`…) reste gérée par `MCP_CONFIG_PATH` / `MCP_SERVERS_JSON`. Nous utilisons désormais `@jsr2npm/yao__gpt-vis-mcp@latest` (voir `plan/Z/mcp.config.json`) pour déléguer les rendus au serveur SSR privé exposé sur le port 3000 (`docker run -p 3000:3000 -e RENDERED_IMAGE_HOST_PATH=http://localhost:3000/charts ghcr.io/yaonyan/gpt-vis-mcp:latest-http`), avec `VIS_REQUEST_SERVER=http://localhost:3000/generate` pour forcer le routage local. Le backend reconnaît les noms de serveur `chart`, `mcp-server-chart` ou `gpt-vis-mcp`.
