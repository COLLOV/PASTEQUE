...
- La configuration du serveur (`VIS_REQUEST_SERVER`, `SERVICE_ID`…) reste gérée par `MCP_CONFIG_PATH` / `MCP_SERVERS_JSON`. Le fichier `plan/Z/mcp.config.json` lance désormais directement le conteneur Docker `ghcr.io/yaonyan/gpt-vis-mcp:latest-mcp` (mode stdio) via `docker run --rm -i --network host …` pour éviter l’installation via `npx`. Le même endpoint SSR privé (`docker run -p 3000:3000 -e RENDERED_IMAGE_HOST_PATH=http://localhost:3000/charts ghcr.io/yaonyan/gpt-vis-mcp:latest-http`) reste requis, avec `VIS_REQUEST_SERVER=http://localhost:3000/generate`. Le backend reconnaît toujours les serveurs `chart`, `mcp-server-chart` ou `gpt-vis-mcp`.
