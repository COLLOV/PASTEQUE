...
5. Les dictionnaires de données (YAML) utilisés par les agents NL→SQL sont lus par défaut depuis `data/dictionary` (répertoire racine du projet) via la variable `DATA_DICTIONARY_DIR` documentée dans `backend/README.md`. Si une table active n'a pas de fichier YAML correspondant, le flux NL→SQL est bloqué avec un message expliquant quelles tables manquent de dictionnaire.
