...
- Les requêtes Cypher exécutées sur Neo4j sont normalisées côté backend : les comparaisons sur les champs de type date (`creation_date`, `date_feedback`, `last_seen`, etc.) transforment automatiquement les littéraux ISO en `date('YYYY-MM-DD')` pour éviter tout décalage entre les métadonnées et les types réels.
